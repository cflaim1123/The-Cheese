...
//My attempt
MS_5803::MS_5803(uint16_t Resolution, TwoWire *bus, uint8_t SDA, uint8_t SCL) {
...
boolean MS_5803::initializeMS_5803(boolean Verbose) {
    ...
    for (int i = 0; i < 8; i++ ){
    	...
    	_busPointer->beginTransmission(MS5803_I2C_ADDRESS);
    	_busPointer->write(0xA0 + (i * 2));
    	_busPointer->endTransmission();
    	_busPointer->requestFrom(MS5803_I2C_ADDRESS, 2);
    	while(_busPointer->available()) {
    		HighByte = _busPointer->read();
    		LowByte = _busPointer->read();
    	}
...
unsigned long MS_5803::MS_5803_ADC(char commandADC) {
	...
	_busPointer->beginTransmission(MS5803_I2C_ADDRESS);
    _busPointer->write(CMD_ADC_CONV + commandADC);
    _busPointer->endTransmission();
    ...
    _busPointer->beginTransmission(MS5803_I2C_ADDRESS);
    _busPointer->write((byte)CMD_ADC_READ);
    _busPointer->endTransmission();
    // Then request the results. This should be a 24-bit result (3 bytes)
    _busPointer->requestFrom(MS5803_I2C_ADDRESS, 3);
    while(_busPointer->available()) {
    	HighByte = _busPointer->read();
    	MidByte = _busPointer->read();
    	LowByte = _busPointer->read();
    }
