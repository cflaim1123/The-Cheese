...
uint8_t broadcastAddress[] = {0x84, 0xF7, 0x03, 0xEC, 0xE5, 0x66};
...
void OnDataRecv(const uint8_t * mac, const uint8_t *incomingData, int len) {
  ...
  Serial.print("Picture flag:");
  Serial.println(sensorData.e);
  Serial.print("Elapsed time: ");
  Serial.println(sensorData.f);
  Serial.println();

  String tempDataRow = String(sensorData.b) + "," + String(sensorData.c) + "," + sensorData.d + 
  "," + String(sensorData.a) + "," + String(sensorData.e) + "," + String(sensorData.f);
  const char* dataRow = tempDataRow.c_str();
  appendFile(SD_MMC, "/data.csv",dataRow);

  // Take and Save Photo
  if(sensorData.e == true){
    // Path where new image will be saved in MicroSD card
    String path = "/image_" + String(sensorData.b) +  ".jpg";
...
// Camera Flashes every time the SD card is accessed
...
// Camera does not flash everytime the SD card is used
void initMicroSDCard() {
 // Start the MicroSD card
 Serial.println("Mounting MicroSD Card");
...
void writeToDataFile(String data)
{
  ...
  if (dataFile) // it opened OK
    {
    Serial.println("Writing to dataFile.csv");
...
void setup() {
  ...
  dataFile = SD_MMC.open(fileName, FILE_WRITE);
  String tempHeader = "Sample #, Pressure (mbar), Temperature (deg C), Time (yyyy-MM-dd HH:mm:ss), Picture sample (T/F), Elapsed time since power on";
